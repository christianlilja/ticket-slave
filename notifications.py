...
def send_email_notification(subject, body, to_email):
    ...
    # Log retrieved settings (except password)
    logger.info(f"SMTP Settings: server={smtp_server}, port={smtp_port}, user={smtp_user}, TLS={use_tls}, from_email={from_email}, password={'<hidden>' if smtp_password else ''}")

    # Validate minimum SMTP config
    if not smtp_server:
    ...
    if not from_email:
        from_email = 'no-reply@36chambers.network'

    try:
        msg = MIMEText(body)
        msg['Subject'] = subject
        msg['From'] = formataddr(("36Chambers Ticket System", from_email))
        msg['To'] = to_email
