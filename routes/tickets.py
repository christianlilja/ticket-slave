    ...
    try:
        ...
        for file_attach in raw_attachments:
            ...
            try:
                ...
                current_app.logger.warning(log_msg, extra={**log_extra, 'attachment_id': file_attach['id'], 'error': str(e)})
                file_dict['size_kb'] = None
            attachments.append(file_dict)

    except Exception as e:
        current_app.logger.error(f"Error fetching ticket details for ticket_id {ticket_id}: {e}", extra=log_extra, exc_info=True)
        flash("Error loading ticket details.", "danger")
...
def update_assigned_to(ticket_id):
    ...
    log_extra_base = {'user_id': user_id, 'username': username, 'ticket_id': ticket_id}

    if new_assigned_user_id_str and new_assigned_user_id_str.strip():
        try:
            ...
            if not user_exists:
                ...
                current_app.logger.warning(f"Update assignment failed: User ID {val_user_id} DNE.", extra=log_extra_base)
            ...
            current_app.logger.warning(f"Update assignment failed: Invalid user ID '{new_assigned_user_id_str}'.", extra=log_extra_base)
            return redirect(url_for('tickets_bp.ticket_detail', ticket_id=ticket_id))
    ...
    try:
        ...
        current_app.logger.error("Error updating ticket assignment", extra=log_extra_update, exc_info=True)
...
def create_ticket():
    user_id = session.get('user_id')
    username = session.get('username')
    log_extra_base = {'user_id': user_id, 'username': username}
...
def add_comment(ticket_id):
    ...
    if not content:
        ...
        try:
            ...
            current_app.logger.error("Error adding comment", extra=log_extra, exc_info=True)
...
def update_status(ticket_id):
    ...
    user_id = session.get('user_id')
    username = session.get('username')
    log_extra = {'user_id': user_id, 'username': username, 'ticket_id': ticket_id, 'new_status': new_status}
    ...
    if not new_status or new_status not in allowed_statuses:
        ...
        try:
            ...
            current_app.logger.error("Error updating ticket status", extra=log_extra, exc_info=True)
...
def update_priority(ticket_id):
    ...
    user_id = session.get('user_id')
    username = session.get('username')
    log_extra = {'user_id': user_id, 'username': username, 'ticket_id': ticket_id, 'new_priority': new_priority}
    ...
    if not new_priority or new_priority not in allowed_priorities:
        ...
        try:
            ...
            current_app.logger.error("Error updating ticket priority", extra=log_extra, exc_info=True)
...
def download_attachment(filename):
    ...
    log_extra = {'user_id': user_id, 'username': username, 'filename': filename, 'ticket_id_from_filename': ticket_id_from_filename}
    ...
    try:
        ...
        current_app.logger.error("Attachment download failed: File not found", extra=log_extra)
        abort(404)
    except Exception as e:
        current_app.logger.error("Attachment download failed (unexpected)", extra=log_extra, exc_info=True)
        abort(500)
